...
def highest_density(
    flow, theta_test, x_test=None, n_samples=1000, nflows_flow=True, sbibm=False
):
        ...
        if x_test is None:
            ...
            for theta_0, x_0 in zip(theta_test, x_test):
                ...
                if not nflows_flow:
                    samples = samples[0]
                repeat_dims = tuple(
                    [samples.shape[0]] + [1 for _ in range(x_0.ndim - 1)]
                )
                mask = flow.log_prob(theta_0, context=x_0) < flow.log_prob(
                    samples, context=x_0.repeat(repeat_dims)
                )
                rank = mask.sum() / mask.numel()
                ranks.append(rank)
...
def hpd(
    ...
    with torch.no_grad():
        if x_test is None:
            for theta_0 in theta_test:
                samples = flow_sample_fn(n_samples)
                mask = flow_log_prob_fn(theta_0[None, :]) < flow_log_prob_fn(samples)
                rank = mask.sum() / mask.numel()
                ranks.append(rank)
        else:
            for theta_0, x_0 in zip(theta_test, x_test):
                theta_0, x_0 = theta_0[None, :], x_0[None, :]
                samples = flow_sample_fn(n_samples, x_0)
                mask = flow_log_prob_fn(theta_0, x_0) < flow_log_prob_fn(samples, x_0)
                rank = mask.sum() / mask.numel()
                ranks.append(rank)
